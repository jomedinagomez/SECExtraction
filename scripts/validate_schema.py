...
with open("analyzers/sec_financial_tables_v1.json", "r", encoding="utf-8") as f:
    tmpl = json.load(f)

tmpl["models"] = {"completion": os.environ["GPT41_MODEL_DEPLOYMENT"], "embedding": os.environ["EMBEDDING_MODEL_DEPLOYMENT"]}

poller = client.begin_create_analyzer(analyzer_id="sec_financial_tables_v2_test", resource=tmpl, allow_replace=True)
result = poller.result()
print("OK:", result.analyzer_id if hasattr(result, "analyzer_id") else "created")

# cleanup
client.delete_analyzer("sec_financial_tables_v2_test")
print("deleted test analyzer")
